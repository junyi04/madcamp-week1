...
## ⚠️ 프로젝트 한계점 및 해결 방향 (Fact Check)
사용자님, 현재 시스템은 매우 강력하지만 실제 운영 시 발생할 수 있는 3가지 허점과 해결 방안을 미리 정리해 두었습니다.   
| 허점 (Hole) | 상세 내용 | 해결 방향 |
|---|---|---|
|API 비용 관리|수집 데이터 폭증 시 구글 API 비용 발생 가능성|한글 정규표현식 검사 후 '애매한 텍스트'만 구글에 보내는 하이브리드 로직 적용|
|메모리 오버헤드|수천 장의 이미지를 동시에 행렬 연산할 때 RAM 부족|Batch Processing을 통해 메모리 점유율을 일정하게 유지하도록 최적화|
|중복 판별 사각지대|썸네일은 다르지만 영상 내용이 100% 같은 경우|향후 영상의 중간 프레임(5초, 10초 지점)을 추가 추출하여 비교하는 로직 고려|
## 📝 설치 및 실행 (Setup)
1. 가상환경 설정
~~~ 
python -m venv .venv
